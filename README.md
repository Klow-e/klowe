...
> VecVecProd(list[list[float]], list[list[float]])  == multiplies the values of two list[list[float]] together
...
> BagFrequency("<text>") -> list[tuple[str,float]]  == relative frequency of each bagword in a text
...
> KSampleTexts() -> list[str]                       == returns every text name in sampletexts.py
